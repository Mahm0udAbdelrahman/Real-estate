...
@section('content')
    ...
    <div class="d-md-flex d-block align-items-center justify-content-between page-header-breadcrumb">
        <div>
            <h2 class="main-content-title fs-24 mb-1">Update Rent Material</h2>
            <ol class="breadcrumb mb-0">
                <li class="breadcrumb-item"><a href="javascript:void(0)">Dashboard</a></li>
                <li class="breadcrumb-item active" aria-current="page">Rent Material</li>
            </ol>
        </div>
    ...
    <div class="row row-sm">
        <div class="col-xl-12">
            <div class="card custom-card">
                ...
                <form class="form-horizontal" method="POST" action="{{ route('RentMaterial.update', $RentMaterial->id) }}"
                    ...
                    <div class="card-body">
                        ...
                        <div class="input-group mb-3">
                            <input type="file" class="form-control" value="{{ $RentMaterial->image }}" name="image"
                                id="basic-url" aria-describedby="basic-addon3">
                        </div>
                        @error('image')
                            <div class="alert alert-danger" role="alert">{{ $message }}</div>
                        ...
                        <div class="input-group mb-3">
                            <input type="text" class="form-control" value="{{ $RentMaterial->price }}" name="price"
                                id="basic-url" aria-describedby="basic-addon3">
                        </div>
                        @error('price')
                            <div class="alert alert-danger" role="alert">{{ $message }}</div>
                            ...
                            <div class="row row-sm">
                                ...
                                <div class="col-md-9">
                                    <select class="form-select" name="status">
                                        ...
                                        <option @if ($RentMaterial->status == '1') selected @endif value="1">
                                            {{ __('Active') }}
                                        </option>
                                        <option @if ($RentMaterial->status == '0') selected @endif value="0">
                                            {{ __('Unactive') }}
                                        </option>
                        ...
                        <div class="card hk-dash-type-1 overflow-hidden">
                            ...
                            <div class="tab-content" id="nav-tabContent">
                                @foreach (LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                                    @php
                                        $translations = $RentMaterial->getTranslationsArray()[$localeCode] ?? [];
                                    @endphp
                                    <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}"
                                        id="pane-{{ $localeCode }}" role="tabpanel"
                                        aria-labelledby="tab-{{ $localeCode }}">
                                        <div class="form-group">
                                            <label for="name-{{ $localeCode }}">{{ __('Name') }}
                                                {{ $localeCode }}</label>
