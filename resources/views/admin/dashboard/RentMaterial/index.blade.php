                ...
                <div class="card custom-card">
                    ...
                    <div class="card-body">
                        <table id="responsivemodal-DataTable" class="table table-bordered text-nowrap" style="width:100%">
                            <a href="{{ route('RentMaterial.create') }}" class="btn btn-primary">Create</a>
                            <thead>
                                <tr>
                                    <th>#</th>
                                    <th>{{ __('Name') }}</th>
                            ...
                            <tbody>
                                @foreach ($RentMaterials as $RentMaterial)
                                    <tr>
                                        <td>{{ $loop->index + 1 }}</td>
                                        ...
                                        <td>
                                            {{ \Illuminate\Support\Str::limit($RentMaterial->bio, 50) }}
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            @foreach ($RentMaterial->getMedia('logo') as $imgs)
                                                <img src="{{ $imgs->getUrl() }}" width="75" height="75"
                                                    alt="">
                                            @endforeach
                                        </td>
                                        <td>
                                            @foreach ($RentMaterial->getMedia('image') as $imgs)
                                                <img src="{{ $imgs->getUrl() }}" width="75" height="75"
                                                    alt="">
                                            @endforeach
                                        </td>
                                        ...
                                        <td>
                                            <div class="d-flex">
                                                <a href="{{ route('RentMaterial.edit', $RentMaterial->id) }}"
                                                    class="btn btn-primary me-2"><i class="las la-edit"></i></a>
                                                <form method="POST"
                                                    action="{{ route('RentMaterial.destroy', $RentMaterial->id) }}">
                                                    @csrf
                                                    @method('delete')
