                ...
                <div class="card custom-card">
                    ...
                    <div class="card-body">
                        <table id="responsivemodal-DataTable" class="table table-bordered text-nowrap" style="width:100%">
                            <a href="{{ route('asks.create') }}" class="btn btn-primary">Create</a>
                            <thead>
                                <tr>
                                    <th>#</th>
                                    <th>Title</th>
                            ...
                            <tbody>
                                @foreach ($asks as $ask)
                                    <tr>
                                        ...
                                        <td>
                                            @php
                                                $trans = App\Models\Translation::where('model_id', $ask->id)
                                        ...
                                        <td>
                                            @php
                                                $trans = App\Models\Translation::where('model_id', $ask->id)
                                                    ...
                                                    ->first();
                                            @endphp
                                            {!! $trans->translate ?? "No Content" !!}
                                        </td>

                                        <td>
                                            @if ($ask->status == 1)
                                                <span class="badge bg-success mb-1">{{ __('Active') }}</span>

                                            @else
                                                  <span class="badge bg-secondary mb-1">{{ __('Unactive') }}</span>
                                            @endif
                                        </td>
                                        <td>
                                            <div class="d-flex">
                                                ...
                                                <form method="POST"
                                                    action="{{ route('asks.destroy', $ask->id) }}">
                                                    @csrf
                                                    @method('delete')
