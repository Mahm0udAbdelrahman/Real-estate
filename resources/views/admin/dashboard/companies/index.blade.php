                ...
                <div class="card custom-card">
                    ...
                    <div class="card-body">
                        <table id="responsivemodal-DataTable" class="table table-bordered text-nowrap" style="width:100%">
                            <a href="{{ route('companies.create') }}" class="btn btn-primary">{{ __('Create') }}</a>
                            <thead>
                                <tr>
                                    <th>#</th>
                                    <th>{{ __('Country') }}</th>
                            ...
                            <tbody>
                                ...
                                @foreach ($companies as $company)
                                    <tr>
                                        ...
                                        <td>{{ $company->{'name' . app()->getLocale()} ?? $company->name}}</td>

                                        <td>{{ $company->phone }}</td>
                                        <td>{{ $company->{'address' . app()->getLocale()} ?? $company->address}}</td>

                                        <td>{{ $company->{'description' . app()->getLocale()} ?? $company->description}}</td>

                                        <td>
                                            @foreach ($company->getMedia('image') as $imgs )
