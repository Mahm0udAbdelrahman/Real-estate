                ...
                <div class="card custom-card">
                    ...
                    <div class="card-body">
                        <table id="responsivemodal-DataTable" class="table table-bordered text-nowrap" style="width:100%">
                            <a href="{{ route('conferences.create') }}" class="btn btn-primary">{{ __('Create') }}</a>
                            <thead>
                                <tr>
                                    <th>#</th>
                                    <th>{{ __('Name Admin') }}</th>
                                    <th>{{ __('Name conference') }}</th>
                                    <th>{{ __('Date') }}</th>
                                    <th>{{ __('Time From') }}</th>
                                    <th>{{ __('Time To') }}</th>
                                    <th>{{ __('Location') }}</th>
                            ...
                            <tbody>
                                ...
                                @foreach ($conferences as $conference)
                                    <tr>
                                        <td>{{ $loop->index + 1 }}</td>
                                        <td>{{ $conference->admin_id }}</td>
                                        <td>{{ $conference->name }}</td>
                                        <td>{{ $conference->date }}</td>
                                        <td>{{ $conference->time_from }}</td>
                                        <td>{{ $conference->time_to }}</td>
                                        <td>{{ $conference->location }}</td>
                                        <td> {{ \Illuminate\Support\Str::limit($conference->about, 50) }}</td>

                                        @foreach ($conference->getMedia('image') as $imgs)
                                            <td>
                                                <img src="{{ $imgs->getUrl() }}" width="75" height="75"
                                                    alt="">
                                            </td>
                                        @endforeach
                                        <td>{{ $conference->rate }}</td>
