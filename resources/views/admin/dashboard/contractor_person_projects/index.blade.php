                ...
                <div class="card custom-card">
                    ...
                    <div class="card-body">
                        <table id="responsivemodal-DataTable" class="table table-bordered text-nowrap" style="width:100%">
                            <a href="{{ route('contractor_person_projects.create') }}" class="btn btn-primary">{{ __('Create') }}</a>
                            <thead>
                                <tr>
                                    <th>#</th>
                                    <th>{{ __('Title') }}</th>
                            ...
                            <tbody>
                                ...
                                @foreach ($ContractorPerson as $project)
                                    <tr>
                                        <td>{{ $loop->index + 1 }}</td>
                                        <td>{{ $project->title }}</td>
                                        <td>
                                         @foreach ($project->getMedia('logo') as $imgs )
                                              <img src="{{ $imgs->getUrl() }}" width="75" height="75" alt="">
                                              @endforeach
                                            </td>
                                        <td>
                                         @foreach ($project->getMedia('image') as $imgs )
                                              <img src="{{ $imgs->getUrl() }}" width="75" height="75" alt="">
                                              @endforeach
                                            </td>

                                      <td> {{ \Illuminate\Support\Str::limit($project->content, 50) }}</td>
                                      ...
                                      <td>{{ $project->rate }}</td>
                                        ...
                                        <td>
                                            ...
                                            <td>
                                                <div class="d-flex">
                                                    <a href="{{ route('contractor_person_projects.edit', $project->id) }}"
                                                        class="btn btn-primary me-2"><i class="las la-edit"></i></a>
                                                    <form method="POST"
                                                        action="{{ route('contractor_person_projects.destroy', $project->id) }}">
                                                        @csrf
                                                        @method('delete')
