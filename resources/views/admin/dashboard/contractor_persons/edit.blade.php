    ...
    <div class="row row-sm">
        <div class="col-xl-12">
            <div class="card custom-card">
                ...
                <form class="form-horizontal" method="POST" action="{{ route('ContractorPersons.update' , $contractorperson->id) }}"
                    ...
                    <div class="card-body">
                        <div class="form-group">
                            <div class="row">
                                <div class="col-md-3">
                                    <label class="form-label">{{ __('Company') }} (option)</label>
                                </div>
                                <div class="col-md-9">
                                    <select class="form-select" name="company_id">
                                        <option disabled selected>{{ __('Choose Company...') }}</option>
                                        @foreach ($companies as $company)
                                         <option @selected($contractorperson->company_id == $company->id ) value="{{ $company->id }}">
                                         {{ App::getLocale() == 'en' ? $company->getTranslationsArray()['en']['name'] : $company->getTranslationsArray()['ar']['name'] }}
                                    </option>
                                        @endforeach
                                    </select>
                                    @error('company_id')
                    ...
                    @enderror
                                ...
                                <div class="col-md-9">
                                    <select class="form-select" name="specialty_id">
                                        ...
                                        @foreach ($specialties as $specialty)
                                         <option @selected($contractorperson->specialty_id == $specialty->id ) value="{{ $specialty->id }}">
                        ...
                        <div class="input-group mb-3">
                            <input type="email" class="form-control" value="{{ $contractorperson->email }}" name="email"
                        ...
                        <div class="input-group mb-3">
                            <input type="text" class="form-control" value="{{ $contractorperson->phone }}" name="phone"
                                id="basic-url" aria-describedby="basic-addon3">
                        </div>
                        @error('phone')
                            <div class="alert alert-danger" role="alert">{{ $message }}</div>
                        ...
                        <div class="input-group mb-3">
                            <input type="text" class="form-control" value="{{ $contractorperson->year_of_experience }}" name="year_of_experience"
                                id="basic-url" aria-describedby="basic-addon3">
                        </div>
                        ...
                        <div class="input-group mb-3">
                            <input type="file" class="form-control" value="{{ $contractorperson->image }}" name="image"
                                id="basic-url" aria-describedby="basic-addon3">
                        </div>
                        @error('image')
                            <div class="alert alert-danger" role="alert">{{ $message }}</div>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-9">
                                    <select class="form-select" name="status">
                                        ...
                                        <option @if ($contractorperson->status == '1') selected @endif value="1">
                                            {{ __('Active') }}
                                        </option>
                                        <option @if ($contractorperson->status == '0') selected @endif value="0">
                                            {{ __('Unactive') }}
                                        </option>
                                    </select>
                                    @error('status')
                        ...
                        <div class="card hk-dash-type-1 overflow-hidden">
                            ...
                            <div class="tab-content" id="nav-tabContent">
                                @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                                @php
                                $translations = $contractorperson->getTranslationsArray()[$localeCode] ?? [];
                                @endphp
                                    <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" id="pane-{{ $localeCode }}"
                                        role="tabpanel" aria-labelledby="tab-{{ $localeCode }}">
                                        <div class="form-group">
                                            <label for="contractor_person_name-{{ $localeCode }}">{{ __('Contractor Person Name') }} {{ $localeCode }}</label>
                                            <input type="text" value="{{ $translations['contractor_person_name'] ?? '' }}"
                                                name="{{ $localeCode }}[contractor_person_name]" id="contractor_person_name-{{ $localeCode }}"
                                                class="form-control">
                                            @error("$localeCode.contractor_person_name")
                                                <span class="text-danger">{{ $message }}</span>
                                            @enderror
                                        </div>

                                        <div class="form-group">
                                            <label for="contractor_person_address-{{ $localeCode }}">{{ __('Contractor Person Address') }} {{ $localeCode }}</label>
                                            <input type="text" value="{{ $translations['contractor_person_address'] ?? '' }}"
                                                name="{{ $localeCode }}[contractor_person_address]" id="contractor_person_address-{{ $localeCode }}"
                                                class="form-control">
                                            @error("$localeCode.contractor_person_address")
                                                <span class="text-danger">{{ $message }}</span>
                                            @enderror
                                        </div>
