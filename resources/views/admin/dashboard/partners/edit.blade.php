...
@section('content')
    ...
    <div class="d-md-flex d-block align-items-center justify-content-between page-header-breadcrumb">
        <div>
            <h2 class="main-content-title fs-24 mb-1">{{ __('Update Partner') }}</h2>
    ...
    <div class="row row-sm">
        <div class="col-xl-12">
            <div class="card custom-card">
                ...
                <form class="form-horizontal" method="POST" action="{{ route('partners.update', $partner->id) }}"
                    ...
                    <div class="card-body">
                                ...
                                <div class="col-md-9">
                                    <select class="form-select" name="specialty_id">
                                        ...
                                        @foreach ($specialties as $specialty)
                                            <option @selected($partner->specialty_id == $specialty->id) value="{{ $specialty->id }}">
                        ...
                        <div class="input-group mb-3">
                            <input type="text" class="form-control" value="{{ $partner->location }}" name="location"
                        ...
                        <div class="input-group mb-3">
                            <input type="text" class="form-control" value="{{ $partner->year_of_experience }}"
                        ...
                        <div class="input-group mb-3">
                            <input type="file" class="form-control" value="{{ $partner->image }}" name="image"
                                id="basic-url" aria-describedby="basic-addon3">
                        </div>
                        @error('image')
                            <div class="alert alert-danger" role="alert">{{ $message }}</div>
                        ...
                        <div class="card hk-dash-type-1 overflow-hidden">
                            ...
                            <div class="tab-content" id="nav-tabContent">
                                @foreach (LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                                    @php
                                        $translations = $partner->getTranslationsArray()[$localeCode] ?? [];
                                    @endphp
                                    <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}"
                                        id="pane-{{ $localeCode }}" role="tabpanel"
                                        aria-labelledby="tab-{{ $localeCode }}">
                                        <div class="form-group">
                                            <label for="name-{{ $localeCode }}">{{ __('Name Partner') }}
                                                {{ $localeCode }}</label>
