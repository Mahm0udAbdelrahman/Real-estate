                ...
                <div class="card custom-card">
                    ...
                    <div class="card-body">
                        <table id="responsivemodal-DataTable" class="table table-bordered text-nowrap" style="width:100%">
                            <a href="{{ route('partners.create') }}" class="btn btn-primary">Create</a>
                            <thead>
                                <tr>
                                    <th>#</th>
                            ...
                            <tbody>
                                @foreach ($partners as $partner)
                                    <tr>
                                        <td>{{ $loop->index + 1 }}</td>
                                        <td>{{ $partner->membership_no  }}</td>
                                        <td>{{ $partner->{'name' . app()->getLocale()} ?? $partner->name }}</td>
                                        <td>{{ $partner->specialty->name ?? '' }}</td>
                                        <td>{{ $partner->country->name ?? '' }}</td>
                                        <td>{{ $partner->city->name ?? '' }}</td>
                                        <td>{{ $partner->location }}</td>
                                        <td>{{ $partner->phone }}</td>
                                        <td>{{ $partner->email }}</td>
                                        <td>{{ $partner->year_of_experience }}</td>
                                        @foreach ($partner->media as $imgs)
                                            <td>
