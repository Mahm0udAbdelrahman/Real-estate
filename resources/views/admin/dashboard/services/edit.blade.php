...
@section('content')
    ...
    <div class="d-md-flex d-block align-items-center justify-content-between page-header-breadcrumb">
        <div>
            <h2 class="main-content-title fs-24 mb-1">{{ __('Update Service') }}</h2>
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            <div class="row">
                                ...
                                <div class="col-md-9">
                                    <select class="form-select" name="subspecialty_id">
                                        <option disabled selected>{{ __('Choose Subspecialty...') }}</option>
                                        @foreach ($subspecialties as $subspecialty)
                                         <option @selected($service->subspecialty_id == $subspecialty->id ) value="{{ $subspecialty->id }}">
                                         {{ App::getLocale() == 'en' ? $subspecialty->getTranslationsArray()['en']['name'] : $subspecialty->getTranslationsArray()['ar']['name'] }}
                                    </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-9">
                                    <select class="form-select" name="status">
                                        ...
                                        <option @if ($service->status == '1') selected @endif value="1">
                                            {{ __('Active') }}
                                        </option>
                                        <option @if ($service->status == '0') selected @endif value="0">
                                            {{ __('Unactive') }}
                                        </option>
                                    </select>
                                    @error('status')
                        ...
                        <div class="card hk-dash-type-1 overflow-hidden">
                            ...
                            <div class="tab-content" id="nav-tabContent">
                                @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                                @php
                                $translations = $service->getTranslationsArray()[$localeCode];
                                @endphp
