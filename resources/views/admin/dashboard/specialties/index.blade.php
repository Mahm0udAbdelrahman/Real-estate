                ...
                <div class="card custom-card">
                    ...
                    <div class="card-body">
                        <table id="responsivemodal-DataTable" class="table table-bordered text-nowrap" style="width:100%">
                            ...
                            <div class="mb-3">
                                <a href="{{ route('specialties.create') }}" class="btn btn-primary">{{ __('Create') }}</a>
                            </div>
                            <div class="mb-3">
                                <a href="{{ route('subspecialties.index') }}" class="btn btn-success">{{ __('Sub Specialties') }}</a>
                            </div>
                            ...
                            <tbody>
                                @foreach ($specialties as $specialtie)
                                    <tr>
                                        <td>{{ $loop->index + 1 }}</td>
                                        <td>{{ $specialtie->{'name' . app()->getLocale()} ?? $specialtie->name}}</td>
                                      <td>
                                            @if ($specialtie->status == 1)
                                                {{ __('Active') }}
                                            @else
                                                {{ __('Unactive') }}
                                            @endif
                                        ...
                                        <td>
                                            <div class="d-flex">
                                                ...
                                                <form method="POST"
                                                    action="{{ route('specialties.destroy', $specialtie->id) }}">
                                                    @csrf
                                                    @method('delete')
